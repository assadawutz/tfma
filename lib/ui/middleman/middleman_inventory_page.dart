...
class _MiddlemanInventoryPageState extends State<MiddlemanInventoryPage> {
      ...
      builder: (context, _) {
        ...
        return MiddlemanScreenScaffold(
          ...
          children: [
            ...
            _buildInventorySearch(filteredLots.length, lots.length),
            const SizedBox(height: 12),
            ..._buildStorageGrid(filteredLots, hasAnyLots: hasAnyLots),
            const MiddlemanSection(
  ...
  Widget _buildInventorySearch(int resultCount, int totalCount) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        MiddlemanSearchField(
          controller: _searchController,
          hintText: 'ค้นหาด้วยชื่อไซโลหรือหมายเลขคลัง',
          onChanged: (value) => setState(() => _searchTerm = value.trim()),
        ),
        const SizedBox(height: 8),
        Text(
          _searchTerm.isEmpty
              ? 'แสดงทั้งหมด $totalCount พื้นที่'
              : 'ผลการค้นหา $resultCount พื้นที่',
  ...
  List<Widget> _buildStorageGrid(List<InventoryLot> lots,
      {required bool hasAnyLots}) {
    if (lots.isEmpty) {
      final message = hasAnyLots && _searchTerm.isNotEmpty
          ? 'ไม่พบพื้นที่จัดเก็บที่ตรงกับ "$_searchTerm"'
          ...
          child: Column(
            ...
            children: [
              ...
              LinearProgressIndicator(
                value: lot.capacityTons <= 0
                    ? 0.0
                    : (lot.filledTons / lot.capacityTons)
                        .clamp(0.0, 1.0)
                        .toDouble(),
  ...
  List<InventoryLot> _filterLots(List<InventoryLot> lots) {
    ...
    return lots
        .where((lot) =>
            '${lot.siloName} ${lot.capacityTons.toStringAsFixed(1)} ${lot.filledTons.toStringAsFixed(1)}'
                .toLowerCase()
                .contains(query))
        .toList();
  }
  ...
  Future<void> _confirmDeleteLot(InventoryLot lot) async {
    final shouldDelete = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('ลบพื้นที่จัดเก็บ'),
        content: Text('ต้องการลบ ${lot.siloName} ออกจากระบบหรือไม่?'),
        actions: [
          TextButton(
