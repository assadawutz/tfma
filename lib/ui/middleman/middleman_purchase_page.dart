...
class _MiddlemanPurchasePageState extends State<MiddlemanPurchasePage> {
  ...
  Widget _buildPurchaseSearch() {
    final matches = _filteredPurchases().length;
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        MiddlemanSearchField(
          controller: _searchController,
          hintText: 'ค้นหาด้วยเลขที่ใบรับซื้อ ชื่อเกษตรกร หรือหมู่บ้าน',
          onChanged: (value) => setState(() => _searchTerm = value.trim()),
        ),
        const SizedBox(height: 8),
        Text(
          _searchTerm.isEmpty
              ? 'แสดงทั้งหมด $matches รายการ'
              : 'พบผลการค้นหา $matches รายการ',
          style: const TextStyle(
            fontSize: 12,
            color: MiddlemanPalette.textSecondary,
          ),
  ...
  List<Widget> _buildPurchaseHistory() {
    ...
    if (purchases.isEmpty) {
      return [
        Container(
          ...
          child: Text(
            _searchTerm.isEmpty
                ? 'ยังไม่มีประวัติการรับซื้อ ระบบจะแสดงรายการเมื่อมีการบันทึก'
                : 'ไม่พบใบรับซื้อที่ตรงกับ "$_searchTerm"',
    ...
    return [
      for (final ticket in purchases)
        MiddlemanListTile(
          leadingIcon: Icons.receipt_long,
          iconColor: MiddlemanPalette.primary,
          title: '${ticket.ticketId} • ${ticket.farmerName}',
          subtitle:
              '${ticket.village}\n${ticket.weightKg.toStringAsFixed(0)} กก. • ฿${ticket.totalPrice.toStringAsFixed(2)}',
          trailing: Column(
            crossAxisAlignment: CrossAxisAlignment.end,
            children: [
              ElevatedButton(
                onPressed: () => _loadPurchase(ticket),
                style: ElevatedButton.styleFrom(
                  backgroundColor: MiddlemanPalette.primary,
                  minimumSize: const Size(120, 36),
                  shape: RoundedRectangleBorder(
                ...
                child: const Text('แก้ไข/นำเข้าฟอร์ม'),
              ),
              TextButton(
                onPressed: () => _confirmDeletePurchase(ticket),
                child: const Text(
                  'ลบ',
  ...
  List<PurchaseTicket> _filteredPurchases() {
    final query = _searchTerm.toLowerCase();
    return _repository.purchases.where((ticket) {
      if (query.isEmpty) {
        return true;
      }
      final haystack =
  ...
  void _loadPurchase(PurchaseTicket ticket) {
    setState(() {
      _reservedQueue = null;
      _ticketIdController.text = ticket.ticketId;
      _farmerController.text = ticket.farmerName;
      _villageController.text = ticket.village;
      _weightController.text = ticket.weightKg.toStringAsFixed(0);
      _priceController.text = ticket.pricePerKg.toStringAsFixed(2);
      _grade = ticket.grade;
    });
  ...
  Future<void> _confirmDeletePurchase(PurchaseTicket ticket) async {
  ...
  void _submitForm() {
    ...
    final result = _repository.recordPurchase(
      ticketId: ticketId,
      ...
      SnackBar(
        content: Text(
          result == WorkflowMutationResult.created
              ? 'บันทึกใบรับซื้อ $ticketId เรียบร้อย'
              : 'อัปเดตรายการใบรับซื้อ $ticketId เรียบร้อย',
        ),
      ),
    );
    _reservedQueue = null;
